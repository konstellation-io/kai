...
minikube_start() {
  ...
  case $MINIKUBE_STATUS in
    ...
    Stopped)
      ...
      minikube start -p "$MINIKUBE_PROFILE" --kubernetes-version="$MINIKUBE_KUBERNETES_VERSION" --insecure-registry "10.0.0.0/24"
    ;;
    *)
...
get_admin_api_pod() {
  kubectl -n ${NAMESPACE} get pod -l app.kubernetes.io/name=kai-admin-api -o custom-columns=":metadata.name" --no-headers
}
