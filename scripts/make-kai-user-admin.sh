#!/bin/sh
# ----
# File:        make-kai-user-admin.sh
# Description: Script to make an existing user ADMIN on the KAI database.
...
PASS="$(
  kubectl get -n kai secrets/mongodb-database-admin-password \
...
if [ "$PASS" ]; then
  ...
  OUTPUT="$(
    kubectl exec -ti pod/mongodb-database-0 -c mongod -- mongo --quiet \
      -u admin -p "$PASS" --authenticationDatabase admin kai --eval "$CMND"
  )"
  if [ "$OUTPUT" ]; then
    echo "$OUTPUT"
  else
